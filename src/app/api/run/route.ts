...
export async function POST(_req?: Request | NextResponse, _ctx?: unknown, extraDays = 0) {
  ...
  // 수집 기사 원본은 최신 run에만 보관 (이전 run은 정리)
  await supabase.from('run_logs').update({ raw_articles: null }).neq('id', runId).not('raw_articles', 'is', null)
...
async function collectAndSave(runId: string, supabase: ReturnType<typeof createServiceClient>, extraDays = 0) {
  ...
  console.log(`[수집완료] runId=${runId}, 수집=${raw.length}건 → 2단계(처리)는 클라이언트 트리거 대기`)
}
